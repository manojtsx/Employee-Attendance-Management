...
interface NepaliDateTimeInputProps {
  value?: Date;
...
const NepaliDateTimeInput = ({ 
  value, 
  onChange, 
  placeholder = "Select Nepali date and time...", 
  className,
  disabled = false,
  showTime = true
}: NepaliDateTimeInputProps) => {
  ...
  const [selectedTime, setSelectedTime] = useState<string>(
    value ? value.toTimeString().slice(0, 5) : '00:00'
  );
  ...
  const handleConfirm = () => {
    ...
    const englishDate = selectedDate.getEnglishDate();
    
    // Add time to the English date
    const [hours, minutes] = selectedTime.split(':').map(Number);
    const finalDate = new Date(englishDate);
  ...
  const formatDisplayValue = () => {
    ...
    return showTime ? `${dateStr} ${selectedTime} BS` : `${dateStr} BS`;
  };
...
export default NepaliDateTimeInput;
